    ...
    def make_layer(input_to_layer):
        with tf.variable_scope(name, values=[input_to_layer]):
            with tf.name_scope('weights'):
                weights = weight_variable([5, 5, input_to_layer.get_shape()[3], 64], stddev=5e-2)
...
def norm_layer(name='norm1-layer'):
    def make_layer(input_to_layer):
        return tf.nn.local_response_normalization(input_to_layer, depth_radius=4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name=name)
    return make_layer
